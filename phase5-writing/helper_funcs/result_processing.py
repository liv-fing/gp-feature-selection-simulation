...
def process_pymc_results(outdir, trace, X):
    ...
    ell_is_vector = 'ell' in posterior_ds and 'ell_dim_0' in posterior_ds['ell'].dims
    ...
    for i in range(num_features):
        ...
        if 'ell' not in posterior_ds:
            ell_i = None
        elif ell_is_vector:
            ell_i = posterior_ds['ell'].isel(ell_dim_0=i).values
        else:
            ell_i = posterior_ds['ell'].values  # same scalar for all features
...
def make_trace_plots(outdir, trace, X, mechanism):
    ...
    ell_is_vector = 'ell' in posterior and 'ell_dim_0' in posterior['ell'].dims
    panels = []
    ...
    if plot_ell and "ell" in posterior:
        if ell_is_vector:
            ...
            panels.append(('ell_scalar', 'ell'))  # single panel for scalar ell
