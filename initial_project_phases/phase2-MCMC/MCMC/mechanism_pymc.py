...
def make_run_dir(sampler="pymc", numchains=2, numsteps=10000, numburn=100, seed=1):
    path =  f"results/{sampler}/w{numchains}_s{numsteps}/sd{seed}"
...
def main(numchains = 50, numsteps = 10000, numburn = 1000, seed = 1):
    ...
    input:
    - numchains: int
        Number of chains for the sampler.
    - numsteps : int
    ...
    output:
    - creates a structured output directory organized by sampler, numchains, and numsteps:
        results/
            {sampler_name}/
                    w{numchains}_s{numsteps}/
                        - metadata.json          # contains sampler, numchains, numsteps, timestamp
    ...
    # set up sampler
    sampler_name = "pymc"
    outdir = make_run_dir(sampler=sampler_name, numchains=numchains, numsteps=numsteps, numburn=numburn, seed=seed)
    random.seed(seed) 
    ...
    with open(os.path.join(outdir, "summary.txt"), "w") as f:
        ...
        f.write(f"Num chains: {numchains}, steps: {numsteps}, burn-in: {numburn}\n")
    ...
    for i in range(X.shape[1]):
        ax = axes[i]
        traces = clean_tab[f"beta{i}"].to_numpy().reshape(numchains, numsteps - numburn).T
...
if __name__ == "__main__":
    ...
    parser.add_argument('--numchains', type=int, default=50, help='Number of chains (default: 2)')
