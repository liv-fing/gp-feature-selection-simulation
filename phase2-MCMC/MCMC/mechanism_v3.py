...
def make_run_dir(sampler="pymc", numdraws=2000, numtune=1000, numchains = 6, steptype = 'Metropolis', seed=1, test_lambda=False, fixed_lambda_val=0):
    if test_lambda:
        path =  f"results/v3/{sampler}/lambda_testing/dr{numdraws}_t{numtune}/lbda{fixed_lambda_val}/ch{numchains}_{steptype}/sd{seed}"
        
    else:
        path =  f"results/v3/{sampler}/dr{numdraws}_t{numtune}/ch{numchains}_{steptype}/sd{seed}"
